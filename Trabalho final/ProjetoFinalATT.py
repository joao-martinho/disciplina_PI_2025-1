...
class FloodAndLandslideProcessor:
    ...
    def add_legend_and_stats(self, image, stats):
        ...
        # Calcula altura necessária para a legenda (incluindo alertas)
        base_legend_height = 160  # Altura base para título e itens da legenda
        alert_height = len(alerts) * 25  # 25 pixels por alerta
        legend_height = base_legend_height + alert_height + 23  # +20 para margem extra

        # Ajusta largura da legenda
        legend_width = 450
        ...
        for terrain_type in self.color_mappings:
            if y_offset > height - alert_height - 50:  # Deixa espaço para os alertas
                break
        ...
        # Adiciona linha separadora se houver alertas
        if alerts:
            separator_y = height - alert_height - 35
            cv2.line(image, (20, separator_y), (legend_width - 10, separator_y), (0, 0, 0), 1)

        # Adiciona alertas na parte inferior da legenda
        alert_y = height - alert_height - 10
        for alert in alerts:
